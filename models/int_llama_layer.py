...
# class OutlierAwareQuantizerWrapper(nn.Module):
#     """
#     Lite-MicroScopiQ Wrapper:
#     1. Pruning Mask: 强制置 0 (Mask-Aware Training 的关键)
#     2. Outlier Mask: 强制保留原始精度 (FP16/BF16)
#     3. Others: 走正常的 Quantizer (LWC 会优化这部分)
#     """
#     def __init__(self, original_quantizer, outlier_mask, pruning_mask=None):
#         super().__init__()
#         self.quantizer = original_quantizer
#         # 使用 register_buffer 确保 mask 会随模型保存移动，但不作为可训练参数
#         self.register_buffer('outlier_mask', outlier_mask)
#         self.register_buffer('pruning_mask', pruning_mask)

#     def forward(self, x):
#         # [Innovation: Decoupled LWC]
#         # 1. 在送入 quantizer 之前，先剔除 Outlier！
#         # 我们把 Outlier 的位置临时替换成 0 或者均值，
#         # 这样 LWC 的 learnable bound 就不会被 Outlier 撑大了。
#         # 注意：这里需要 detach() 还是保留梯度？
#         # 策略：我们希望 LWC 只优化 Inlier。
        
#         # 构造一个只包含 Inlier 的输入 x_inliers
#         # 这里的 mask=1 是 Outlier (FP16)，mask=0 是 Inlier
#         # 我们把 Outlier 位置的值设为 0 (或者当前层的 mean，0比较安全且对LWC无害)
#         x_inliers = x.masked_fill(self.outlier_mask, 0.0)
        
#         # 2. 仅对 Inliers 进行量化计算 (LWC 此时只会根据 Inliers 调整参数)
#         q_x_inliers = self.quantizer(x_inliers)
        
#         # 3. 组合最终结果
#         # Outlier 位置 -> 保持原始 x (FP16)
#         # Inlier 位置 -> 使用优化后的 q_x_inliers
#         out = torch.where(self.outlier_mask, x, q_x_inliers)
        
#         # 4. 应用剪枝 (Pruning)
#         if self.pruning_mask is not None:
#             out = out.masked_fill(self.pruning_mask, 0.0)
...
class OutlierAwareQuantizerWrapper(nn.Module):
    def __init__(self, original_quantizer, outlier_mask, pruning_mask=None):
        ...
        # [新增] 8-bit 离群值量化参数
        # 既然是 Outliers，它们的数值通常比较大，建议用 Per-Channel 或 Per-Tensor 的 MinMax
        # 这里演示最简单的 Per-Tensor 对称量化 (INT8 range: -127 ~ 127)
        self.outlier_scale = nn.Parameter(torch.ones(1)) 
        self.enable_8bit = True # 开关

    def forward(self, x):
        # 1. 计算 INT4 部分 (Inliers)
        # x_inliers 已经是量化过的 INT4 模拟值
        if self.quantizer.bits == 4:
            # 调用原始量化器 (LWC)
            q_x_inliers = self.quantizer(x)
        else:
            q_x_inliers = x # Fallback

        # 2. 计算 INT8 部分 (Outliers) -- [核心修改]
        if self.enable_8bit:
            # 动态计算 Scale (在训练/校准阶段)
            # 这里的 x 是全精度的，我们只关心 outlier_mask 为 True 的部分
            # 但为了张量操作方便，我们对整个 x 算 scale，或者只统计 masked 部分
            
            # 简单模拟：对称量化
            # quantization range: [-127, 127]
            # scale = max(abs(x_outliers)) / 127
            
            # 在推理时，scale 是固定的。这里演示 Fake Quantization (模拟量化误差)
            # Q = clamp(round(x / scale), -127, 127)
            # D = Q * scale
            
            # 为了简单起见，这里直接用 PyTorch 的 fake_quantize
            # q_x_outliers = torch.fake_quantize_per_tensor_affine(
            #     x, scale=self.outlier_scale, zero_point=0, 
            #     quant_min=-128, quant_max=127)
            
            # 手写一个极简版 (假设 scale 已知或动态计算)
            max_val = x.detach().abs().max()
            scale = max_val / 127.0
            q_int8 = torch.clamp(torch.round(x / scale), -128, 127)
            q_x_outliers = q_int8 * scale
        else:
            # 旧方案：直接保留 FP16
            q_x_outliers = x

        # 3. 组合最终结果
        # 如果是 Outlier -> 用 8-bit 量化后的值
        # 否则 -> 用 4-bit 量化后的值
        out = torch.where(self.outlier_mask, q_x_outliers, q_x_inliers)
...
class QuantLlamaDecoderLayer(nn.Module):
    ...
    # --- Add this method inside QuantLlamaDecoderLayer ---
    # 在 QuantLlamaDecoderLayer 类中更新此方法
    # [Lite-MicroScopiQ Update] 支持迭代式更新掩码
    def register_outlier_masks(self, fisher_pruner, outlier_ratio=0.05, prune_ratio=0.15):
        for name, module in self.named_modules():
            if isinstance(module, QuantLinear):
                # 1. 计算 Mask
                o_mask, p_mask = fisher_pruner.get_block_balanced_masks(
                    name, block_size=16, protect_ratio=outlier_ratio, prune_ratio=prune_ratio
                )
                
                device = module.weight.device
                
                # [关键优化] 直接把剪枝应用到权重上，不保存 p_mask！
                # 这样可以节省 7GB 显存，防止 OOM
                if p_mask is not None:
                    # 将需要剪枝的位置(p_mask为True)强制置为0
                    module.weight.data.masked_fill_(p_mask.to(device), 0.0)
                    # 既然权重已经是0了，就不需要额外的 mask 张量了
                    p_mask = None 

                if o_mask is not None:
                    # 2. 检查是否已经是 Wrapper
                    if isinstance(module.weight_quantizer, OutlierAwareQuantizerWrapper):
                        module.weight_quantizer.outlier_mask = o_mask.to(device)
                        # p_mask 已经被融合并置为 None，这里不需要更新它
                    else:
                        print(f"  -> [Lite-MicroScopiQ] Protecting {outlier_ratio*100}% & Pruning {prune_ratio*100}% (Fused) in {name}")
                        module.weight_quantizer = OutlierAwareQuantizerWrapper(
