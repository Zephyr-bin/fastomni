...
# [Lite-MicroScopiQ Fix] Import Wrapper to check type
from models.int_llama_layer import OutlierAwareQuantizerWrapper
...
# [Lite-MicroScopiQ Modified] smooth_and_quant_temporary
def smooth_and_quant_temporary(model, args, isllama):
    if args.let:
        ...
        if isllama:
            ...
            # Fallback for OPT/Falcon if needed
    ...
    for name, module in model.named_modules():
        if isinstance(module, QuantLinear):
            # 1. Perform standard quantization
            if hasattr(module, "temp_weight"):
                target_weight = module.temp_weight
            else:
                target_weight = module.weight

            # [Innovation] Check for Wrapper and Outlier Mask
            if isinstance(module.weight_quantizer, OutlierAwareQuantizerWrapper):
                    # Call internal quantizer explicitly
                    quantized_weight = module.weight_quantizer.quantizer(target_weight)
                    
                    # Manual Restoration logic
                    mask = module.weight_quantizer.outlier_mask
                    if mask is not None:
                        if mask.device != quantized_weight.device:
                            mask = mask.to(quantized_weight.device)
                        # Restore outliers: use original target_weight where mask is True
                        module.temp_weight = torch.where(mask, target_weight, quantized_weight)
                    else:
                        module.temp_weight = quantized_weight
            else:
                # Standard path
                module.temp_weight = module.weight_quantizer(target_weight)
...
# [Lite-MicroScopiQ Modified] smooth_and_quant_inplace
@torch.no_grad()   
def smooth_and_quant_inplace(model, args, isllama):
    ...
    for name, module in model.named_modules():
        if isinstance(module, QuantLinear):
            # [Innovation] Inplace Restoration
            original_weight = module.weight.clone()
            
            if isinstance(module.weight_quantizer, OutlierAwareQuantizerWrapper):
                quantized_weight = module.weight_quantizer.quantizer(original_weight)
                mask = module.weight_quantizer.outlier_mask
                if mask is not None:
                    if mask.device != quantized_weight.device: mask = mask.to(quantized_weight.device)
                    module.weight = torch.where(mask, original_weight, quantized_weight)
                else:
...
def set_quant_state(model, weight_quant: bool = False, act_quant: bool = False):
    ...
    # self.use_weight_quant = weight_quant  <-- Removed self
    # self.use_act_quant = act_quant        <-- Removed self
